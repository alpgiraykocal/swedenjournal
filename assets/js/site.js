...
function responsiveImage(p, opts={}){
  ...
  return `<picture>${avifSet?`<source type="image/avif" srcset="${esc(avifSet)}" sizes="${sizes}">`:""}${webpSet?`<source type="image/webp" srcset="${esc(webpSet)}" sizes="${sizes}">`:""}<img${className} src="${esc(fallback)}"${jpegSet?` srcset="${esc(jpegSet)}" sizes="${sizes}"`:""} alt="${esc(alt)}"${loading} decoding="async"${size}></picture>`;
}
...
function storyHref(slug){ return `${root()}story/?slug=${encodeURIComponent(slug)}`; }
...
function storyShareUrl(data, story){
  const base = String(data.site?.baseUrl || "").replace(/\/+$/,"");
  if(base) return `${base}/story/?slug=${encodeURIComponent(story.slug)}`;
  const url = new URL(location.href);
  url.searchParams.set("slug", story.slug);
  return url.href;
}
...
function injectPreload(p, sizes, fallbackSize="medium"){
  ...
  link.rel = "preload";
  link.as = "image";
  link.href = variant(p, fallbackSize, "jpeg") || imgPath(p);
  link.setAttribute("data-preload-photo", p.id);
  const avif = srcset(p, "avif");
  const webp = srcset(p, "webp");
  const jpeg = srcset(p, "jpeg");
  if(avif) link.setAttribute("imagesrcset", avif);
  else if(webp) link.setAttribute("imagesrcset", webp);
  else if(jpeg) link.setAttribute("imagesrcset", jpeg);
...
function header(data){
  ...
  return `<a class="skip-link" href="#app">Skip to content</a><header class="site-header"><div class="container nav"><a class="brand" href="${root()}index.html">${esc(data.site.ownerName)}</a><nav class="nav-links" aria-label="Primary navigation">
...
function storyCard(data,s){
  ...
  return `<a class="card story-card" href="${storyHref(s.slug)}" data-story-card data-category="${esc(storyCategory(s))}"><div class="story-card-media"${mediaRatioStyle(p)}>${responsiveImage(p,{className:"story-card-img",sizes:"(max-width: 850px) calc(100vw - 28px), 48vw"})}</div><div class="story-card-copy"><span class="meta">${esc(storyCardMeta(s))}</span><h3>${esc(s.title)}</h3><p class="muted">${esc(s.summary)}</p>${strip?`<span class="story-strip" aria-label="Story photographs">${strip}</span>`:""}${tags.length?`<span class="story-tags">${tags.map(t=>`<span>${esc(t)}</span>`).join("")}</span>`:""}<span class="story-link">Read story${s.readingTime?` · ${esc(s.readingTime)}`:""}</span></div></a>`;
}
function featuredStoryBlock(data, story){
  const p = storyPhotos(data, story)[0];
  return `<a class="featured-story" href="${storyHref(story.slug)}"><div class="featured-story-media"${mediaRatioStyle(p)}>${responsiveImage(p,{className:"story-card-img",priority:true,sizes:"(max-width: 850px) calc(100vw - 28px), 620px"})}</div><div class="featured-story-copy"><span class="meta">${esc(storyCardMeta(story))}</span><h3>${esc(story.title)}</h3><p class="muted">${esc(story.summary)}</p><span class="story-link">Read featured${story.readingTime?` · ${esc(story.readingTime)}`:""}</span></div></a>`;
}
...
function photoFigure(p, options={}){
  ...
  return `<figure class="photo-card ${p.featured ? "featured" : ""}" data-photo-id="${esc(p.id)}" data-tags="${esc((p.tags||[]).join('|'))}" data-theme="${esc(p.theme||"")}"><div class="photo-media"${mediaRatioStyle(p)}>${responsiveImage(p,{priority:opts.priority,eager:opts.eager,sizes:opts.sizes || "(max-width: 560px) calc(100vw - 24px), (max-width: 900px) 50vw, 380px"})}${button}</div><figcaption><strong>${esc(p.title)}</strong>${p.location?` — ${esc(p.location)}`:""}${p.caption?`<br>${esc(p.caption)}`:""}</figcaption></figure>`;
}
function blockHtml(data, block){
  if(!block) return "";
  if(block.type === "image"){
    const p = photo(data, block.photoId);
    return p ? `<figure class="story-inline-photo">${responsiveImage(p,{className:"story-inline-img",sizes:"(max-width: 920px) calc(100vw - 40px), 840px",fallbackSize:"full"})}<figcaption>${esc(block.caption || p.caption || "")}</figcaption></figure>` : "";
  }
...
async function boot(){
  try{
    ...
    const preloadMap = {
      ...
      story: photo(data, ((data.stories || []).find(x => x.slug === query().get("slug")) || data.stories?.[0])?.heroPhotoId)
    };
...
function renderStory(data){
  ...
  updateMeta(data, {title:s.title, description:s.summary || data.site?.description, path:`story/?slug=${encodeURIComponent(s.slug)}`, imagePhoto:p});
  $("#app").innerHTML = `<main><section class="story-hero container"><div class="story-meta" aria-label="Story details">${storyMetaChips(s)}</div><h1 class="headline">${esc(s.title)}</h1><p class="intro">${esc(s.summary)}</p>${responsiveImage(p,{priority:true,sizes:"(max-width: 1220px) calc(100vw - 40px), 1180px",fallbackSize:"full"})}</section><article class="story-body">${(s.body||[]).map(block=>blockHtml(data,block)).join("")}</article>${relatedPanel(data,s)}${sharePanel(data,s)}</main>`;
  bindShareControls();
}
...
function relatedPanel(data, story){
  ...
  const relatedStories = (data.stories || []).filter(item => item.slug !== story.slug).map(item => {
    const score = (item.tags || []).filter(tag => storyTags.has(String(tag).toLowerCase())).length + (storyCategory(item) === storyCategory(story) ? 4 : 0);
    return {item, score};
...
function bindGalleryControls(data, list){
  ...
  const update = (active, syncUrl=true) => {
    ...
    let visible = 0;
    cards.forEach(card => {
      const themeTokens = (card.dataset.theme || "").split(/[,/]/).map(normalize).filter(Boolean);
      const tagTokens = (card.dataset.tags || "").split("|").map(normalize).filter(Boolean);
      const tokens = [normalize(card.dataset.theme), ...themeTokens, ...tagTokens].filter(Boolean);
...
function bindLightbox(data, getVisiblePhotos){
  ...
  const open = (p, mode="push") => {
    ...
    media.innerHTML = responsiveImage(p,{className:"lightbox-img",priority:true,sizes:"1200px",fallbackSize:"full"});
