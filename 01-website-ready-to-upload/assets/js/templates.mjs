...
export function storiesMain(data) {
  ...
  const mapSection = mapStories.length ? `<section class="section stories-map-section"><div class="container"><div class="section-head"><div><p class="eyebrow">Explore by place</p><h2>Story map</h2></div></div><div id="storiesMap" class="stories-map" role="region" aria-label="Map of story locations"></div><script type="application/json" id="stories-map-data">${JSON.stringify(mapStories).replace(/</g, "\\u003c")}</script></div></section>` : "";
...
export function aboutMain(data) {
  ...
  return `<main><section class="hero container about-grid">${responsiveImage(p, { className: "about-img", priority: true, sizes: "(max-width: 850px) calc(100vw - 28px), 520px" })}<div><p class="eyebrow">${esc(a.eyebrow)}</p><h1 class="headline">${esc(a.headline)}</h1><div class="prose">${(a.paragraphs || []).map((x) => `<p>${esc(x)}</p>`).join("")}</div>${a.contactEmail ? `<div class="about-contact"><span class="eyebrow">Get in touch</span><a href="mailto:${esc(a.contactEmail)}">${esc(a.contactEmail)}</a></div>` : ""}</div></section></main>`;
}
